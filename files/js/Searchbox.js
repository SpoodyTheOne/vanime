...
function drawSearchCanvas() {
    ...
    if ((mouse.clicked || mouse.rightclicked) && !Loading && (AnimeList.length > 0 || EpisodeList.length > 0)) {
                ...
                if (!ShowingEpisodes) {
                    ...
                    CurrentAnime?.getEpisodes().then((episodes) => {
                        ...
                        EpisodeList.forEach((episode) => {
                            setTimeout(() => {
                                episode.download();
                                DownloadedAnime[CurrentAnime.name].seasons[episode.season].episodes[episode.episode] = true;
                            }, 500 * index);
                            index++;
                        });
                        Loading = false;
                    ...
                    Loading = true;
                    EpisodeList[Math.round(EpisodeIndex)]?.download().finally(() => {
                        Loading = false;
                    });
                }
            } else {
    ...
    if (Loading) {
        ...
        ctx.fillText("Loading...", canvas.width / 2, canvas.height / 2, canvas.width - 20);
        ...
        for (let k = 0; k < 10; k++) {
            ...
            if (!anime) continue;
            //try {
            let x = canvas.width / 2 - width / 2 + (width + 20) * (i - AnimeIndex);
            let y = canvas.height / 2 - height / 2 + 15 * Math.abs(i - AnimeIndex);
        ...
        ctx.font = "25px Arial";
        ctx.fillStyle = "#ecf0f1";
        ctx.fillText(AnimeList[Math.round(AnimeIndex)]?.name || "", canvas.width / 2, 90, canvas.width - 20);
    } else {
        for (let k = 0; k < 10; k++) {
            ...
            let x = canvas.width / 2 - width / 2 + (width + 20) * (i - EpisodeIndex);
            let y = canvas.height / 2 - height / 2 + 15 * Math.abs(i - EpisodeIndex);
        ...
        ctx.font = "25px Arial";
        ctx.fillStyle = "#ecf0f1";
        ctx.fillText(EpisodeList[Math.round(EpisodeIndex)]?.name || "", canvas.width / 2, 90, canvas.width - 20);
    }
...
function ChangeIndex(amount) {
    if (ShowingEpisodes) {
        ...
        for (let i = 0; i < Steps; i++) {
            setTimeout(() => {
                EpisodeIndex = Math.max(Math.min(EpisodeIndex + dif, EpisodeList.length - 1), 0);
            }, time * i);
        }
    } else {
        ...
        for (let i = 0; i < Steps; i++) {
            setTimeout(() => {
                AnimeIndex = Math.max(Math.min(AnimeIndex + dif, AnimeList.length - 1), 0);
            }, time * i);
        }
    }
}
...
function IsDownloaded(video) {
    return DownloadedAnime[video?.anime?.name]?.seasons[video?.season]?.episodes[video?.episode] ? true : false;
}
