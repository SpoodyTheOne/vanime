...
function drawSearchCanvas() {
    ...
    if (Loading) {
        ...
        for (let i = 0; i < AnimeList.length; i++) {
            ...
            ctx.drawImage(anime.image, canvas.width / 2 - width / 2 + (width + 20) * (i - AnimeIndex), canvas.height / 2 - height / 2 + 15 * Math.abs(i - AnimeIndex), width, height);
        }

        ctx.textAlign = "center";
        ctx.font = "25px Arial";
        ctx.fillStyle = "#ecf0f1";
        ctx.fillText(AnimeList[Math.round(AnimeIndex)]?.name || "", canvas.width / 2, 120, canvas.width - 20);
    } else {
        for (let k = 0; k < 10; k++) {
        ...
        ctx.font = "25px Arial";
        ctx.fillStyle = "#ecf0f1";
        ctx.fillText(EpisodeList[Math.round(EpisodeIndex)]?.name || "", canvas.width / 2, 120, canvas.width - 20);
    }
